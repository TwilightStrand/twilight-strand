...
local function readLUT(seed, nodeID, jewelType)
	...
	if jewelType == 1 then
		if not data.timelessJewelLUTs[jewelType] or not data.timelessJewelLUTs[jewelType].data or not next(data.timelessJewelLUTs[jewelType].data) then
			return { }
		end
	else
		if not data.timelessJewelLUTs[jewelType] or not data.timelessJewelLUTs[jewelType].data then
			return { }
		end
	end
	 -- "Elegant Hubris"
