...
export const APIRoute = createAPIFileRoute("/api/og")({
  GET: async ({ request }) => {
    ...
    const stats: string[] = [];
    if (parseInt(dps) > 0) stats.push(`${fmtNum(dps)} DPS`);
    if (parseInt(life) > 1) stats.push(`${fmtNum(life)} Life`);
    if (parseInt(es) > 0) stats.push(`${fmtNum(es)} ES`);

    // SVG-based OG image (TODO: use satori for proper PNG if needed)
  ...
  <text x="600" y="420" text-anchor="middle" fill="#06b6d4" font-family="monospace" font-size="32">${escapeXml(stats.join("  ·  "))}</text>
</svg>`;
